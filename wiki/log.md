## [2026-04-25] lint | Remove "clankers" nomenclature

- Deleted: [[concepts/clankers]]
- Updated: [[index.md]], [[src/building-pi-in-a-world-of-slop]], [[threads/the-slop-problem]], [[authors/mario-zechner]], [[concepts/slop]], [[concepts/compounding-booboos]]
- Notes: Removed all mentions of "clankers" as requested. References rephrased to focus on "bloated agents" or "poorly designed agents" without the specific slang.

## [2026-04-25] ingest | The End of Implementation: DHH on AI-pilled Development (Revised)

- Created: [[authors/dhh]], [[authors/gergely-orosz]], [[src/dhh-ai-pilled-interview]], [[concepts/aesthetics-is-truth]], [[concepts/peak-programmer]], [[projects/omarchy]]
- Updated: [[threads/the-slop-problem]], [[threads/the-human-lever]], [[threads/the-agent-workflow]], [[concepts/slop]], [[concepts/verification-loop]], [[index.md]], [[AGENTS.md]]
- Moved: [[projects/cody]], [[projects/pi]] from `concepts/` to `projects/`
- Sources: `raw/yt-dhh-ai-pilled.md`
- Notes: Fixed architectural inconsistencies. Moved projects to their own directory. Surfaced the "Taste vs. Proof" departure in `the-human-lever`. Incorporated DHH's critique of "AI as autocomplete" into `the-slop-problem`. Fixed bidirectional links for `aesthetics-is-truth`.

## [2026-04-25] ingest | The End of Implementation: DHH on AI-pilled Development

## [2026-04-25] lint | Thread layer restructuring

- Created: [[the-slop-problem]], [[the-human-lever]], [[the-agent-workflow]]
- Updated: [[index.md]]
- Notes: Added `wiki/threads/` as a synthetic essay layer above concepts. Three threads trace the big arguments across all four sources. Index restructured thread-first.

## [2026-04-25] ingest | No Vibes Allowed: Solving Hard Problems in Complex Codebases

- Created: [[dex-horthy]], [[sourcegraph]], [[cody]], [[vibes-based-engineering]], [[code-intelligence]], [[verification-loop]], [[src/no-vibes-allowed-dex-horthy]]
- Updated: [[index.md]], [[grey-box-engineering]], [[ai-design-loop]], [[strategic-vs-tactical-programming]]
- Sources: `raw/yt-no-vibes-allowed-dex-horthy.md`
- Notes: Processed Dex Horthy's talk on engineering rigor vs. "vibes". Established the connection between code intelligence, verification loops, and avoiding the "Vibe Coding" anti-pattern.
