...
- `raw/memrefine-llm-guided-compression-for-long-term-agent-memory.md` — Kim (Korea U), Baek, Jeong, Hwang (KAIST; Hwang also DeepAuto.ai), June 2026. *MemRefine: LLM-Guided Compression for Long-Term Agent Memory.* Adds a **post-construction memory compression** primitive to the memory taxonomy. The compressor solves a query-agnostic max-min program by iterating a pairwise LLM judge (DELETE/MERGE/PRESERVE on factual content) over the store. Framework-agnostic: A-MEM graph memory and Mem0. LLM judge decisively outperforms fixed rule-based baselines (RuleSim, RulePR) as the budget tightens. Modest degradation at 30% budget; A-MEM F1 holds to within 3.9pp on standard LoCoMo.
