...
> [[dex-horthy|Dex Horthy]] is the wiki's strongest first-person source on the dark-factory endgame because he built one and shut it down (July→November 2025). The mechanism is the slop spiral this thread documents, run to completion: agents ship faster than humans can review → review becomes the bottleneck → replace review with agentic review/testing → but those reviewers have no architecture intuition (no benchmark trains 3–6-month maintainability) → the codebase becomes easier to rewrite than fix. His users-didn't-complain oracle is the weakest-possible verification signal — exactly the verifiability gap slop exploits. See [[dark-factory]] and [[dex-horthy-agentic-engineering]].
...
The named articulation of the concept comes from [[vibv|Vaibhav Gupta]]'s (Boundary) conference talk titled literally *fighting slop with slop* (`raw/fighting-slop-with-slop-vaibhav-gupta-boundary.md`), which the thread previously knew through the Boundary AI livestream. The talk adds the primary articulation: slop is defined reading-relative ("Slop is just any code you don't read") and monotonic ("this is the least amount of slop that your code base will ever have"). Boundary ships a programming language under a no-code-review regime — every engineer works in parallel, no AI standardization — substituting an invariant toolchain for review ([[context-files|`architecture.md`]] instead of `CLAUDE.md`, dependency-graph CI guarantees, agent-transcript inspection, and design docs that must be read). Its concrete countermeasure to slop creep is documented first-hand: the founder's "AI psychosis" (10 design docs/day until "the team was fighting my slop") was re-contained by the rule that shipped design docs must actually be read ([3:39]–[3:58]) — friction allocated at the design layer, not the tooling layer, consistent with [[armin-ronacher|Ronacher]]'s [[deliberate-friction]] finding below.

A productive tension emerges from this practice: slop isn't universally harmful — it can be **channeled**. Per the maintainer's own account ([14:11]–[14:33] in `raw/yt-can-an-ai-out-plan-a-senior-engineer.md`), the BEEPs toolchain at Boundary ML (web UI, Slack integration, CLI, versioning) is pure AI-generated code he has never read: "I don't think I've ever looked at the code in the beeps folder"; "I have never even opened claude myself to add features into beeps." The code is intentionally disposable — never read, never reviewed; features are added by tagging coding agents on Slack.
...
Kevin Gregory frames it: "You're using slop to build these internal tools that make it really easy to get a really high quality document." ([13:29]–[13:41] in `raw/yt-can-an-ai-out-plan-a-senior-engineer.md`)
...
## Foundation-Layer Slop: The Language Itself

Vaibhav Gupta adds a seventh slop category to this thread's taxonomy (after code, benchmark, spec, skill, information, and discourse slop): **slop baked into the language**. The argument ([7:07]–[9:10] in the talk):

- TypeScript's stated design goal — striking a balance between correctness and productivity — is really *human* productivity: "if you were designing for a world where humans never wrote a single line of code," you would never make those tradeoffs.
- Concrete instances: `Array.sort` casting elements to strings; the slide's second example ([8:19]–[8:30]), which the wiki reads as the `any` escape hatch — "this is just slop baked into the language, whether you like it or not" ([8:15]–[8:17]).
- The layering pathology: JavaScript exists, then CoffeeScript, then TypeScript, then more layers — "the thing underneath is already broken" ([8:44]–[9:01]).
- The prescription: "I think we do need a new Git. I think we do need a new database, and yes, I think we need a new programming language" ([21:04]–[21:09]) — foundations designed from first principles for a world where agents write the code.

This is the slop problem's deepest layer: individual teams can contain code slop with discipline, but every team building on a language inherits its language-level slop regardless of discipline. It extends the thread's two existing language arguments — [[armin-ronacher|Ronacher]]'s "language as slop factor" (Go's simplicity reduces agent errors) and [[harness-monoculture|harness monoculture]] (slop propagated through model weights) — to the claim that the substrate itself must be rebuilt, not merely selected or patched.

> [!note] Departure: Foundation Rewrite as an Attributed Prediction, Not a Recommendation
> The talk's own pessimism is the caveat: "we're still going to lose the war" ([7:07]–[7:13]) — asking everyone to rewrite all code into a new system is itself a slop-generating impossibility, which is why BAML's practical path is embedded calls from existing languages rather than a migration ([17:27]–[18:19]). The thread therefore holds the foundation-rewrite argument as an attributed prediction (Vaibhav's), not a recommendation the wiki endorses. The actionable near-term claim is narrower and testable: languages' human-productivity tradeoffs are a slop source agents inherit, and compiler-proven rigidity ([[baml]]'s inferred error types) is the countermeasure.

## Cramer on Junk Code and Security

[[david-cramer|David Cramer]] strengthens this thread from the C-level production-software perspective. His core thesis: "[LLMs] produce junk. Sometimes that's okay. But it is certainly... there's no putting the thing back in the closet." He forced himself to stop writing code by hand almost a year before the interview — he is deeply skeptical of the 100x productivity claims — and his conclusion is blunt: "There's absolutely no way to produce good software which is prompting LLMs, no matter what. It's going to be bloated, overengineered, complex, Java everywhere, Java factory patterns non-stop."
...
- `raw/yt-building-pi-and-what-makes-self-modifying-software-so-fascinating.md` — Agents don't feel pain; training data ceiling; "slow the f down" math; 30-team interview findings; deliberate friction removal as slop accelerant. Two-guest episode (Zechner + Ronacher), unlabeled transcript; speaker attribution based on contextual cues, not verified against audio.
...
- `raw/yt-can-an-ai-out-plan-a-senior-engineer.md` — Fighting slop with slop concept, BEEPs design doc workflow, threading design process; the never-read BEEPs toolchain detail as the maintainer's first-person claim ([14:11]–[14:33]). Multi-speaker livestream; the never-read attribution is the maintainer's own account.
...
- `raw/yt-state-of-agentic-coding-8-with-mario-armin-and-ben.md` — [[mario-zechner|Zechner]] and [[armin-ronacher|Ronacher]] on training-harness slop: Claude Code's lenient harness masking malformed tool calls so models never learn to avoid them; the harness-monoculture thesis that slop propagates through model weights as a de-facto spec. Source for the "Training-Harness Slop" extension callout. Three-guest episode (Zechner, Ronacher, and Ben), unlabeled transcript; speaker attribution based on contextual cues, not verified against audio.
...
- `raw/fighting-slop-with-slop-vaibhav-gupta-boundary.md` — [[vibv|Vaibhav Gupta]] (Boundary): the origin talk for fighting slop with slop (reading-relative slop definition, monotonic slop, no-code-review engineering triad, invariant toolchain, the design-doc reading rule); the foundation-layer slop argument (TypeScript's human-productivity design goal, "slop baked into the language," new Git/database/language prescription). Solo talk; attribution direct.
