...
- [[prototyping]] — Originator of the `/prototype` skill: high-fidelity throwaway code to resolve look/behavior questions that discussion cannot
...
- `raw/yt-dont-waste-time-on-specs-prototype-instead.md` — The `/prototype` skill and the fidelity framework: high-fidelity throwaway prototypes resolve look/behavior questions; Wayfinder as the planning container (grilling + prototype ticket types); UI vs logic prototypes; the throwaway-branch → AFK re-implement handoff; the Shape Up lineage.
