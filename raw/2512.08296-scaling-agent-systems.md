...
1Google Research, 2Google DeepMind, 3Massachusetts Institute of Technology, †Corresponding Author
      ...
      architectural effects. The resulting model achieves a cross-validated   2=0.373 across all six benchmarks
      (  2=0.413 with a task-grounded capability metric). We identify a robust capability-saturation effect and
...
while recent agentic benchmarks have emerged (e.g., SWE-Bench [20], 2-Bench [24], Terminal-
...
and (6) terminal tasks (Terminal-Bench [25]). Across =260 controlled configurations with matched
...
amplification factors, message density and redundancy as predictors, achieving cross-validated
  2=0.373 across all six benchmarks ( 2=0.413 with a task-grounded capability metric), without
...
performance prediction.
    ...
    Our analysis identifies three scaling patterns. First, a tool-coordination trade-off (=-0.096,
 =0.002): tool-heavy tasks (e.g., 16-tool business workflows) suffer from multi-agent coordination
overhead, with efficiency penalties compounding as environmental complexity increases. Second, a
capability ceiling (=-0.236, =0.004): tasks where single-agent performance already exceeds 45%
...
Towards a Science of Scaling Agent Systems
    ...
    ( 2=0.373 across all six benchmarks;         2=0.413 with a task-grounded capability metric) using em-
    pirical coordination metrics, efficiency (), trace-level error amplification (  trace), and redundancy
          () to quantify how performance emerges from the interplay of reasoning capability and task prop-
    erties. This framework identifies fundamental limits on coordination, specifically a tool-coordination
            trade-off (=-0.096) where tool-heavy workflows suffer from coordination tax, and safety bounds
...
Agentic Tasks and Benchmarks We define agentic tasks following Zhu et al. [19] as requiring:
...
Towards a Science of Scaling Agent Systems
    ...
    Building on multi-agent system formalism [14, 19], an agent system S = (, , , Ω) consists of a
    set of agents = {1,. . . ,           } (where ≥ 1), a shared environment , a communication topology ,
    and an orchestration policy Ω. When || = 1, we refer to this as a Single-Agent System (SAS); when
    || > 1, a Multi-Agent System (MAS). Each agent         perceives, reasons, and acts within the shared
    ...
    • A = {ToolCall(,   ) : ∈ T, ∈ Θ} is the action space consisting of tool usage, where T is the set of
       available tools (e.g., web search, code execution) and Θ represents valid parameter configurations
       for tool
    ...
    function   is instantiated by the reasoning policy Φ (the LLM): given a history ℎ,, the LLM generates
    a reasoning trace and selects the next action.

       For instance, a history ℎ, = [("search(query='pandas')", "Found 5 files"), ...] is processed by Φ
    to produce the next tool call ,+1.
       At timestep , agent     selects an action  , ∈ A according to:

            ,     = (ℎ,),     ,     = (,),     ℎ,+1 =     (ℎ,,     ,, ,),

    where      denotes the environment and ℎ,0 = {0} contains the initial task specification. The history
    update function                : H × A × O → H appends the new action-observation pair to the agent's
 history: ℎ,+1 = (ℎ,, ,, ,) = ℎ, ⊕ (,, ,), subject to context window truncation when |ℎ,+1| >
...
Single-Agent System (SAS). A Single-Agent System contains one reasoning locus (|| = 1 where
  is the agent set). All perception, reasoning, and action occur within a single sequential loop,
producing computational complexity () where is the number of reasoning iterations. SAS has
zero communication overhead and minimal memory (), but limited capacity for decomposition or
verification.


             Multi-Agent System (MAS). A Multi-Agent System is an agent system S with || > 1, where agents
    interact through communication topology    and orchestration policy Ω.
...
• Hybrid: = centralized ∪ peer (orchestrator plus limited peer-to-peer)
    ...
    • Independent MAS:     = {1,. . ., }, C = {( ,           agg)}, Ω = synthesis_only. The synthesis_only
    ...
    • Centralized MAS:     = {orch,  1, . . ., },   = {(orch,       ) : ∀}, Ω = hierarchical. A single or-
    chestrator coordinates  rounds across     sub-agents ((              )). Sequential depth equals while
    parallelization factor remains      . This design stabilizes reasoning but creates a bottleneck at the
    orchestrator.
    • Decentralized MAS:       = {1,. . .,  }, = {( ,         ) : ∀, , ≠ }, Ω = consensus. Agents communi-
    cate in sequential debate rounds ((     )). Memory complexity is (          ) as each agent stores its
    own debate history. This enables consensus formation through peer-to-peer discussion.
    • Hybrid MAS: = {orch,  1, . . . ,     },    = star + peer edges, Ω = hierarchical + lateral. Combines
    orchestrated hierarchy with limited peer communication (( ( + ) ·     ·   ) where     is the number of
...
Towards a Science of Scaling Agent Systems
    ...
    and collectively steer problem-solving direction.
                  ...
                  Thus, SAS represents the minimal unit of agentic computation (()), while MAS configurations
    ...
    We formally define the task-level error rate as      = 1-, where    is the fraction of tasks successfully
    resolved. The task-level error amplification factor  task    = MAS/SAS quantifies the relative error rate
    of a multi-agent system compared to its single-agent baseline;   task     > 1 indicates that coordination
    introduces net errors, while  task   < 1 indicates net error suppression. We additionally define a trace-
    level error amplification factor trace        that measures how much extra computational work arises from
    ...
    effectively than independent coordination, though they differ in absolute magnitude ( task      ≈ 1.1-1.3
 vs. trace ≈ 4-17) because they capture complementary aspects of error dynamics.

    3.2. Agentic Tasks and Benchmarks

    Following and extending the framework of Zhu et al. [19], we operationalize a task        as agentic when
    optimal performance substantially benefits from adaptive interaction. Formally, if    = {( , )}        =0
    represents an interaction trajectory, then:
        max     [()] - max     [(())]
        max     [()]     > ,

    where represents an interactive policy, represents any single-forward-pass function, measures
    task success, is a task-dependent threshold, and the expectation is over task instances               and
...
environment interaction.
          ...
          Benchmark Design Principles. Extending the framework proposed by Zhu et al. [19], we introduce
    ...
    • Action-Observation Loop Length: each benchmark enforces non-trivial trajectory length       > 3 to
...
performance? We examine how task structure (e.g., decomposability, tool complexity, sequential
    ...
    Characteristic            SAS             MAS                         MAS                          MAS        MAS
                                         (Independent)              (Decentralized)     (Centralized)                                       (Hybrid)

                                Agent        Agent       Agent    Agent  Agent      Agent              Orchestrator                     Orchestrator
    Interaction Type
...
* = max iterations per agent, = number of agents, = orchestrator rounds, = debate rounds, = peer
communication rounds, = average peer requests per round. Communication overhead counts inter-agent
...
Benchmarks. We conducted 260 experiments across six benchmarks spanning deterministic to
...
configurations (coefficient of variation / = 0.32 computed across all 45 BrowseComp-Plus runs
spanning architectures and model families, with Anthropic models contributing substantial variance
due to lower absolute performance, where is the standard deviation of success rates and is the
...
yielding 8 models per benchmark and a total of =260 configurations. Strong consistency across
...
as max, |ˆarch, - ˆarch, | across families , ∈ {OpenAI, Google, Anthropic}), with coefficient of vari-
...
through parallel agent processing (smaller per-agent iterations for -agent teams), while SAS received
...
synthesis accuracy for BrowseComp-Plus). Secondary metrics include: (i) factual error rate via
domain-specific validators (Cohen's [59]: Finance Agent = 0.91, Workbench = 0.89, PlanCraft
...
Towards a Science of Scaling Agent Systems
    ...
    exhibiting high collinearity with included measures (e.g., total message count correlates     > 0.92
    ...
    • Coordination overhead            = (MAS - SAS)/SAS × 100%: captures computational cost, identified
    as a primary bottleneck in production multi-agent deployments.
        • Message density (inter-agent messages per reasoning turn): quantifies communication intensity,
    a key factor in coordination scaling.
    • Redundancy rate                (mean cosine similarity of agent output embeddings): measures agent
    agreement, relevant for ensemble-based error correction.
    • Coordination efficiency          = /(/SAS) (success normalized by relative turn count): normalizes
    success by cost for deployment decisions.
    • Error amplification   trace  (trace-level error propagation factor, estimated from execution-trace
   token analysis): quantifies how coordination failures compound through agent interactions. The
   complementary task-level metric task = MAS/SAS is defined in Section 3.

    4.2. Main Results

    MAS exhibits domain-dependence with architectural variation. Multi-agent systems show highly
  ...
  Turn 3: craft → Task complete
    ...
    variance ( = 37.5%). The performance range across MAS variants spans from -70.0% (PlanCraft
...
Domain Complexity Moderates Coordination Efficacy. Empirical patterns across benchmarks
...
-39.0% to -70.0%).
   ...
   We characterize each benchmark by a domain complexity score ∈ [0, 1] (Appendix C), capturing
...
strong information-value convergence ( = 0.71, < 0.001), while PlanCraft (sequential constraints)
shows weak correlation ( = 0.18, = 0.22), indicating that agents in high-complexity domains
...
SAS). On Workbench, where multi-agent overhead is less tolerable (efficiency degrades from = 0.466
for SAS to = 0.074 for Hybrid, the largest relative drop across benchmarks), Anthropic's best variant
...
Regression Model Achieves Cross-Validated 2=0.413 (ACI) / 2=0.373 (Intelligence Index).
We fit a scaling principle to all 260 configurations across six benchmarks that relates agentic system
performance to four categories of predictors: 1) base model capability (intelligence index ), 2) system
configuration (agent count ), 3) task properties (tool count , single-agent baseline SA). These are
instance-level predictors capturing within-benchmark variation, distinct from the benchmark-level
domain complexity defined in Appendix C, and 4) empirically measured coordination metrics
from Table 5 (efficiency , overhead %, trace-level error amplification trace, message density ,
redundancy ). Rather than including all possible terms, we construct the model based on specific
mechanistic hypotheses.

    Main effects capture direct relationships between individual factors and performance. We include
a quadratic term ( 2) to test for non-linear capability scaling, and log-transformed tool count and
...
interactions, each motivated by observed patterns: × tests whether efficiency penalties compound

        16

                                    Towards a Science of Scaling Agent Systems

    with tool complexity;    trace × tests whether errors propagate more severely in tool-rich environments;
    SA × log(1 +             ) captures the baseline paradox where high single-agent performance leaves less
    room for coordination gains;    % ×          tests whether overhead costs scale with task complexity. We
    deliberately exclude interactions without clear mechanistic justification (e.g.,       × , × %) to avoid
    overfitting.

    The complete functional form is:

        = 0 +                  1(   - ¯) + 2( - ¯)2 +       3 log(1 + ) + 4 log(1 +   )
                           + 5 log(1 + %) + 6 + 7 + 8                 + 9 log(1 +    trace)
                           + 10 SA + 11( × ) + 12(           trace × SA)
                           + 13(% × ) + 14( × ) + 15( × )
                           + 16(       × ) + 17(SA × log(1 +          ))
                           + 18( × log(1 + )) + 19(         trace × ) + ,                                (1)

        ■ < 0.001 ■ Significant ( < 0.05) ■ Non-significant ( > 0.05)

    where all predictors are standardized ( = 0,              = 1) after transformation. Log transformations
are applied to right-skewed variables spanning multiple orders of magnitude (%: 0-515%; : 2-16;
  : 1-4; trace: 1.0-17.2) to improve approximate linearity and reduce skewness. The trace ×
    interaction retains trace without additional log transformation because log(1+    trace) already appears
    as a main effect; including log(1 + trace) ×      would introduce near-collinearity (VIF > 8, indicating
    substantial multicollinearity). Sensitivity analysis confirms qualitatively consistent results under
    alternative specifications (Δ  2
    ...
    Index as the capability metric, the model achieves  2               2
                                                        train = 0.463,     CV = 0.373 (±0.170 SD). Replacing
    ...
    Index:  = 0.45), improves model fit to     2                  2
train = 0.481,                                                         CV = 0.413 (±0.130 SD), AIC = -244.8,
    ...
    The Efficiency-Tools Interaction Emerges as a Consistent Directional Pattern (ˆ                = -0.096,
    = 0.002). Among the significant interactions, the efficiency-tools trade-off exhibits the largest effect
    size among interaction terms: ˆ × = -0.096 (95% CI: [-0.154, -0.037],         = 0.002). This interaction
    reveals that tool-heavy tasks suffer disproportionately from multi-agent inefficiency. Empirically, single-
    agent systems achieve  = 0.466 (Table 5), while multi-agent architectures range from             = 0.074
    (hybrid) to  = 0.234 (independent), a 2-6× efficiency penalty.
    For a task with     = 16 tools  (e.g., Workbench benchmark), the interaction coefficient ˆ           × =
    ...
    interpreted by directly multiplying raw values of       and    .  Instead, we interpret this coefficient
...
Consistent with this interpretation, simple tasks (                ≤ 4) exhibit negligible efficiency effects
    (|Δ| < 0.05), explaining why multi-agent coordination can succeed on decomposable problems. This
    ...
    reveals dramatic variance in trace-level error amplification factors: single-agent (  trace       = 1.0),
    centralized ( trace = 4.4), decentralized ( trace = 7.8), hybrid ( trace   = 5.1), and independent multi-
    agent ( trace = 17.2).       After controlling for other coordination metrics, neither the main effect of
    error amplification (ˆ = 0.014, = 0.658) nor its interaction with tool count ( trace × : ˆ       = 0.022,
           = 0.332) reaches statistical significance. This suggests that the dramatic performance differences
    across architectures observed in Table 5 are better explained by other coordination mechanisms,
    particularly efficiency () and overhead (%), rather than error propagation per se. Independent
    ...
    opportunities, but this effect is subsumed by the efficiency metric (         = 0.234 for Independent vs.
    = 0.466 for SAS).

    Overhead Scales Non-Linearly with Task Complexity via the % ×     Interaction.                Multi-agent
    ...
    performance. The scaling principle reveals this overhead interacts with tool count (ˆ %×        = -0.033,
        = 0.211), a directional pattern that loses significance in the 6-benchmark model due to the increased
    heterogeneity of task domains. The direction is preserved: for hybrid architecture (% = 515) on
    workbench ( = 16), overhead costs compound with tool complexity, explaining hybrid's collapse
    ...
    naive OLS on the 4-benchmark subset (ˆ     = -0.162,      < 0.001) and report it as a directional pattern
    under the more conservative 6-benchmark specification.

    Intelligence Shows Linear Positive Effect (ˆ     = 0.126, = 0.008).          After centering intelligence
    ...
    The quadratic term ( 2) is not significant (       = 0.977), indicating that capability scaling follows a
    linear rather than accelerating pattern within the tested range ( ∈ [42, 71]). This finding suggests
    ...
    Redundancy Provides Marginal Benefit at Scale (ˆ     ×    = 0.024,     = 0.034).         Work redundancy,
    ...
    positive interaction with agent count (ˆ ×    = 0.024, 95% CI: [0.002, 0.047],       = 0.034), suggesting
    redundancy offers error-correction benefits when more agents participate. For a 4-agent system with
    = 0.50:
  Δredundancy = 0.024 × 0.50 × 4 = 0.048,
    ...
    holdout ( = 260, six benchmarks). The full model with interaction terms achieves the best cross-
    validated fit (  2
    ...
    capture meaningful variance beyond base predictors alone.
                     ...
                     Model Specification            2        2    AIC    Parameters
                                                    train   CV
...
compared to efficiency losses (|ˆ × | = 0.096, 4× larger), indicating redundancy cannot compensate
for architectural inefficiency. The significance ( = 0.034, near the = 0.05 threshold) suggests
...
( = 0.50±0.06), achieves top performance on tool-heavy tasks (workbench success 0.664), consistent
...
paradox: redundancy helps when there is room for improvement (SA < 0.45) but becomes overhead
when baseline is high.


    The Scaling Principle Enables Quantitative Architecture Selection.           Equation 1 synthesizes 20
    parameters into a predictive tool for architecture design. Given task characteristics (,       SA) and
    model capability (), practitioners can compute expected performance for each architecture using
    empirical coordination metrics from Table 5. Consider three task archetypes: (1) Planning tasks
    ( = 4,          SA = 0.57) favor single-agent due to baseline paradox and low tool count; (2) Analysis
    tasks ( = 5, SA = 0.35) favor centralized multi-agent, balancing error control ( trace     = 4.4) with
    manageable overhead; (3) Tool-heavy tasks ( = 16, SA = 0.63) favor decentralized multi-agent
    ...
    Quantitatively, the decision boundary between single-agent and multi-agent is:
                 ...
                 SA = -∗ ˆ4 ≈ 0.040
                         ˆ    0.236 = 0.170         (in standardized units),
                         17
...
Towards a Science of Scaling Agent Systems
    ...
    and empirical coordination metrics (         2
                                                 train = 0.463, 2
                                                      CV = 0.373,                   = 260 configurations across six
    benchmarks, AIC=-236.3). Using the task-grounded ACI improves fit to                 2
    Intelligence is mean-centered (¯                                                         CV = 0.413 (Table 13).
                                         = 57.5) to address multicollinearity between and            2 (VIF reduced
    from 200 to 1.1). Model uses 5-fold cross-validation. Non-significant terms ( > 0.05) indicated with
    †.

      Predictor                     ˆ             95% CI                     Interpretation
      Main Effects
      Intercept (0)               0.430       [0.412, 0.448]     <0.001      Baseline performance
      Intelligence ( - ¯)            0.126    [0.033, 0.218]     0.008       Linear capability effect
      Intelligence2 (( - ¯)2)        -0.000   [-0.019, 0.018]    0.977†      Quadratic capability (not significant)
      log(1 +    )                   0.166    [0.095, 0.236]     <0.001      Tool diversity benefit
      log(1 +     )               0.040       [-0.074, 0.155]    0.487†      Agent count effect
      Single-Agent Baseline (SA)  0.250       [0.102, 0.397]     0.001       Task difficulty proxy
      Coordination Structure
      log(1 +    %)                  0.011    [-0.033, 0.056]    0.611†      Direct overhead cost
      ...
      log(1 +    trace)              0.014    [-0.047, 0.074]    0.658†      Error amplification
      Critical Interactions
      SA × log(1 +     )             -0.236  [-0.396, -0.076]  0.004         Baseline paradox
    ×                                -0.096  [-0.154, -0.037]    0.002       Efficiency-tools trade-off
      % ×                            -0.033   [-0.084, 0.019]    0.211†      Overhead scales with task complexity
      trace ×                        0.022    [-0.023, 0.067]    0.332†      Error propagation in tool-rich systems
      ×                              0.024    [0.002, 0.047]     0.034       Redundancy benefit with scale
      ×                              -0.011   [-0.060, 0.038]    0.653†      Capability-efficiency
      trace  × SA                    -0.080  [-0.148, -0.012]    0.022       Error-baseline
      ×                              -0.016   [-0.059, 0.027]    0.457†      Communication-capability
      × log(1 + )                    -0.051   [-0.113, 0.012]    0.112†      Capability-tools

    Table 5 | Coordination metrics across architectures and families (              = 260 configurations across six
    ...
    and applied uniformly across all benchmarks. All systems matched for total reasoning tokens (mean
      = 4, 800 per trial). Error amplification (          trace) is measured at the trace level via execution-trace
token analysis; the complementary task-level metric task is defined in Section 3.
           ...
           Redundancy ()             0.00        0.48±0.09       0.50±0.06   0.41±0.06           0.46±0.04
           Efficiency ()                0.466     0.234          0.132       0.120           0.074
           Error Amp (     trace)         1.0     17.2           7.8         4.4             5.1
...
response exchanges) exhibit power-law growth with agent count:
     ...
     = 2.72 × ( + 0.5)1.724, 2 = 0.974, 95% CI on exponent : [1.685, 1.763], < 0.001.
...
eter scaling (e.g., Kaplan et al. report = 0.76 for dense models). Empirically, Hybrid systems require
6.2× more turns than SAS (44.3 vs. 7.2 turns; (178) = 16.8, < 0.001), while Centralized requires
...
Message Density Exhibits Logarithmic Saturation with Performance. Across communicating
MAS architectures (excluding configurations with = 0), success rate follows an approximately
logarithmic relationship with message density:

        = 0.73 + 0.28 ln(), 2 = 0.68, < 0.001,

    where is messages per reasoning turn. SAS and Independent configurations are excluded from
this fit because their message density is zero. Performance plateaus near ∗ = 0.39 messages/turn
...
ordination overhead, = 44.3) shows -2.4% versus Centralized (285% overhead, = 27.7), a
difference of 1.1% that is not statistically significant ((178) = 0.61, = 0.542). This saturation
reflects fundamental information limits in open-ended reasoning rather than mechanism failures:
high-performing runs show convergent token overlap (shared tokens: mean ≈ 1.8 bits; < 0.001 vs.
low performers) suggesting message consensus is reached; further messages add redundancy rather
than novel information.


Error absorption mechanisms. We formalize error absorption as Absorb = (SAS - MAS)/SAS,
where is factual error rate. The absorption mechanism operates through iterative verification: in
...
verification. Independent MAS shows no error correction (+4.6% amplification) due to absence of
    ...
    0.5
    0.4        0.37                   0.42    0.37                       0.6     0.50  0.53
           0.34        0.32    0.34                                      0.5         0.47      0.43     0.42
    0.3        0.27  0.21                             0.22      0.23     0.4         0.32        0.37   0.29
    0.2        0.19                                      0.18            0.3
                                                                         0.2         0.18
    0.1                                                                  0.1
    0.0        OpenAI        Anthropic                 Gemini            0.0     OpenAI        Anthropic    Gemini
    ...
    shared-token entropy (mean ≈ 1.8 bits for Finance Agent;                          < 0.001 vs. low-performing runs); (ii)
    ...
    However, high redundancy ( > 0.50) correlates negatively with success ( = -0.136,                              = 0.004),
    implying an emergent diversity-efficiency trade-off: collective capability peaks when message overlap
    balances shared grounding with informational diversity; optimal redundancy occurs at                              ≈ 0.41
...
Accuracy
           ...
           (a)  35     Gemini 2.0 Flash        (b)        Gemini 2.5 Pro
                30                                    50
                25                                    45
                20                                    40
                                                      35
    ...
    agents (  ∈ {1, 3, 5, 7, 9}). Both models show initial gains from multi-agent coordination, but scaling
...
These patterns identify three operational coordination regimes: (i) Under-coordination (                  <
    100% overhead): minimal accuracy gain (Δ ≈ +2-4%), coordination mechanisms not yet engaged;
    (ii) Optimal band (200% <     < 300% overhead): highest success-cost ratio (              ≈ 0.16), dom-
    inated by Centralized and Decentralized, with strong error absorption; (iii) Over-coordination
    (  > 400% overhead): Hybrid runs with reduced efficiency (          ≈ 0.11), protocol complexity intro-
...
Workbench), ΔI correlates strongly with MAS-SAS gap ( = 0.71, < 0.001), indicating that agents
...
Cross-Domain Consistency of Coordination Patterns. Architectural rankings remained stable
across domains (Kendall = 0.89, coefficient of variation < 0.1 across architectures), indicating
coordination principles transcend specific task structures. Extrapolation to larger teams via the fitted
power law predicts ≈69 turns at =6 and ≈157 turns at =10 (95% CI from exponent uncertainty:
...
rather than mechanistic conclusions.
    ...
    We subject the 6-benchmark regression ( = 260) to three robustness checks addressing pseudorepli-
...
Cluster-robust inference. We re-estimated the regression with cluster-robust standard errors
(clustering on dataset, = 6, CR1 correction, 5 critical values). Predictors varying primarily at the
...
than confirmed effects. single_agent_baseline ( = 0.004) and error_x_baseline ( = 0.030)
...
Multiple-comparison correction. We evaluated 19 predictor coefficients (excluding the inter-
cept) as a single family of simultaneous hypotheses, applying the Holm-Bonferroni step-down
procedure to control the family-wise error rate at = 0.05. Three predictors survive correction:
log_tools (Holm < 0.001), single_agent_baseline (Holm = 0.018), and efficiency_x_tools
(Holm = 0.026). Two further predictors (intelligence_centered and baseline_x_agents) are
suggestive (Holm < 0.10) and are discussed as directional patterns (Table 15). The Holm-Bonferroni
correction was applied to naive OLS -values (addressing multiplicity), while the cluster-robust
analysis (addressing pseudoreplication) was reported separately. Under cluster-robust inference alone,
only single_agent_baseline ( = 0.004) survives at = 0.05, making capability saturation the
single most robust finding across both correction approaches.


Capability metric sensitivity. As an alternative to the Intelligence Index, we introduce the Agentic
Capability Index (ACI), defined as each model's mean single-agent performance across all six bench-
marks. The two metrics are only moderately correlated ( = 0.45), validating the concern that static
...
Cross-domain generalization. Leave-one-dataset-out (LODO) cross-validation highlights the chal-
...
configurations on SWE-bench Verified and Terminal-Bench ( < 0.001 by binomial test).
...
comparisons are underpowered at = 20, aggregate trends across 8 models × 2 benchmarks remain
robust (e.g., the 45% threshold achieves 94% match rate, < 0.001 by binomial test). All per-cell
...
interaction) will test whether our observed scaling principles generalize beyond symbolic domains.
    ...
    (vii) Our regression analysis clusters observations at the dataset level ( = 6). With a small number
...
confirmed under both cluster-robust inference ( = 0.004) and Holm-Bonferroni multiple-comparison
correction (Holm = 0.018). Two additional directional patterns emerge consistently across all
...
task decomposability rather than team size. We derive a predictive model ( 2=0.373 across all
six benchmarks; 2=0.413 with a task-grounded capability metric) that achieves 87% accuracy in
...
[24] Victor Barres, Honghua Dong, Soham Ray, Xujie Si, and Karthik Narasimhan.       2-bench: Evalu-
...
[66], AA-LCR [67], Terminal-Bench Hard, and 2-Bench Telecom [24]), with careful standardization,
...
that cannot be fully reproduced. Thus, our estimates provide a methodologically aligned but not
    ...
    Claude Sonnet 4              62∗           ∗
                          47            5           87           75       21     56∗     38∗     35∗
                                 58∗           ∗                                         32∗     30∗
    Claude 3.7 Sonnet     42            2           81           67       12     57
...
Qualitative Findings Validation. Table 9 evaluates whether the five key findings from Section 4
...
specific behavior: Independent MAS degradation validates only for GPT-5.2 but not for Gemini models,
    ...
    Capability Ceiling (higher     correlates with diminishing MAS  ✓                     ✓                  ✓
    returns)
...
Model Family Differences. An informative comparison arises from models with identical Intelligence
Index (Table 10). Despite Index = 75, single-agent performance varies substantially: GPT-5.2
achieves SA = 0.45, while Gemini-3.0 Pro and Gemini-3.0 Flash achieve 0.36 and 0.34, respectively.
...
models (ˆHybrid = 0.560), yet empirically Centralized and Decentralized architectures achieve superior
performance. This discrepancy reflects two factors: (1) linear extrapolation of the intelligence effect
(ˆ = 0.126) beyond its training range, and (2) the model's failure to capture Hybrid's dispropor-
...
Towards a Science of Scaling Agent Systems
    ...
    We characterize domain complexity through an ordinal score         ∈ [0, 1] that captures the degree of
    ...
    Domain complexity        ∈ [0, 1] is assigned based on three empirical task properties, each normalized
    to [0, 1]:
    ...
    SWE-bench Verified  0.255   Decomposable software engineering tasks; multi-step codebase ex-
                                ploration with test feedback; high tool count (7)
    Terminal-Bench      0.414   Diverse CLI tasks with varying difficulty; Docker-based environments;
                                low tool count (2)


C.3. Critical Threshold

Our analysis identifies a critical complexity threshold at ≈ 0.40. Below this threshold, multi-agent
...
Finance Agent. We use the Finance Agent benchmark [33], comprising 50 finance questions
...
BrowseComp Plus. BrowseComp Plus [32] contains 100 web browsing tasks requiring multi-website
...
ground truth answers with confidence scoring.
    ...
    WorkBench. WorkBench [34] evaluates business task automation through function calling se-
...
Plancraft. Plancraft [21] focuses on sequential planning in Minecraft environments. Agents must
...
SWE-bench Verified. SWE-bench Verified [20] evaluates software engineering capabilities through
...
Terminal-Bench. Terminal-Bench [25] evaluates CLI task execution across diverse system adminis-
...
Towards a Science of Scaling Agent Systems
    ...
    from 500), and Terminal-Bench (20 instances, first-        from 86). The smaller subsets for SWE-bench
...
where ∈ {0, 1} is the task success indicator, spre is the agent's state representation before coordination
(initial reasoning trace), and spost is the state after coordination (final aggregated output). Variances
are estimated via Monte Carlo sampling: we generate                 = 10 reasoning traces per state using
temperature           = 0.7 and compute empirical variance of predicted success probabilities. For binary
...
Table 13 | Regression fit under alternative capability metrics (6-benchmark model, = 260). The
Agentic Capability Index (ACI) achieves the best cross-validated fit with zero finding reversals.

                 Capability Metric                  2          2        AIC
                                                    train     CV
    ...
    Table 14 | Naive vs. cluster-robust -values for key predictors (6-benchmark model, = 260,         = 6
...
Table 15 | Holm-Bonferroni multiple comparison correction (19 hypotheses, = 0.05). Three
predictors survive family-wise error rate correction.
    ...
    intervals ( = 20 instances, 10,000 resamples). Values shown as point estimate [ lower, upper ].
